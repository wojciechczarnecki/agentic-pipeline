   ...
   **Wyjątek — pliki w `.claude/`:** ich zapisu z powłoki strażnik nie przepuszcza (to
   ...
   - `.claude/settings.json` — z `templates/settings.json` (permissions allow/ask/deny,
     ...
     **Bez `enabledPlugins`** — sesja w katalogu, który włącza plugin w
     ...
     (`…/<marketplace>/<plugin>/<wersja>/`): ta sama wartość idzie w klucz
     `extraKnownMarketplaces` i w regułę
     `Read(~/.claude/plugins/cache/<marketplace>/pipeline/**)` w `permissions.allow` —
     bez tej reguły etapy nie wczytają szablonów ani mapy sekcji pluginu.
     ...
     `TODO:` przy tej wartości (w obu miejscach) i wymień ją na liście do uzupełnienia
     z kroku 7.
