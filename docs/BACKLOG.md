...
| Guard | `gh api` write calls on the repository endpoint (`gh api -X PATCH repos/<owner>/<repo>`, topics, settings) pass the guard, while `gh repo edit` is refused as the owner's call | The first session seen changing repository settings through `gh api`, or Stage 5 starting | Found in the plan review of SPEC 003 (2026-09-21), which set the *About* description through the owner rather than through this gap; SPEC 003 changes no guard behaviour |
