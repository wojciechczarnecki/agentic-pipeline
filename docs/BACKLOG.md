...
| Guard | Commands the guard cannot see in a command string: scripts written to files and then run, interpreters (`python3 -c`, `node -e`), shell functions defined in earlier calls, and git configuration channels it does not parse (`--config-env`, `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_n`/`GIT_CONFIG_PARAMETERS`, `GIT_CONFIG_GLOBAL`, `include.path`/`includeIf`) | The first session seen reaching a guarded action through one of them | Documented as known limits in `plugin/docs/GUARD.md` (SPEC 002); the guard is best effort, not a sandbox, and on `main` the `pre-push` hook and the rulesets stand behind it. Closing them means analysing file contents or process behaviour, not command strings |
