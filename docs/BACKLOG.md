...
| Reach | Listing in plugin catalogues (claudemarketplaces.com, awesome-claude-plugins lists) and a write-up on the guard as a shell analyser rather than a regex | Stage 4 is done | Visibility is the only lever left once the storefront is in place; `docs/PROJECT.md` lists catalogues as out of scope "for now", so this also needs that line changed |
