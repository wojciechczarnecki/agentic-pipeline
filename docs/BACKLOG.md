...
| Guard | The guard protects configurable release-channel branches (e.g. `protectedBranches` in `.claude/workflow.json`), not only `main`/`master` | The first guard change, or a second release channel | With a `--scope user` install `stable` feeds every project on the machine, and its ruleset's bypass is the owner's account, so an agent pushing with those credentials could move it to any commit. Today only the `deny` rules on `git push` to `stable` in this repository's `.claude/settings.json` stop that — configuration, easy to lose (`claude plugin` commands have stripped blocks from it before), and blind to forms like `git -C . push` |
